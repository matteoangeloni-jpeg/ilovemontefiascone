...
| Core Web Vitals (mobile) | **Lighthouse lab score 71/100** on homepage (accessibility 96, best-practices 92, SEO 100 — checked 2026-07-22 via PageSpeed Insights). **Real-user (CrUX) field data: origin-level 404 — site-wide traffic is still below CrUX's reporting threshold.** The one page with enough individual traffic to qualify (`festa-santa-margherita-montefiascone`, the highest-traffic page in GSC) has genuinely good field data: LCP p75 1,369ms (Good, <2,500ms), CLS p75 0.00 (Good), FCP p75 1,259ms, TTFB p75 376ms. The gap between a middling lab score and excellent real-world data on the one page with real signal suggests the lab score's simulated slow-network throttle is the more pessimistic (and less representative) number — worth improving, but not an active problem for real visitors today. | Lighthouse 80+, re-check CrUX origin-level as FR/traffic grows | Lighthouse 85+ | CrUX origin-level reporting (needs sustained traffic growth) |
