...
> - **Sprint F (UI/UX polish):** consolidate le sezioni "correlate" impilate in 13 hub/pagine in un unico blocco "Continua a esplorare" con griglia responsive (`.related-grid`, h2→h3 sottogruppi), preservando tutti i link; aggiunto schema **`ImageGallery`** valido a `galleria.html` (25 ImageObject reali, didascalie = alt esistenti); CSS additivo (degrada con grazia). Nessuna ricompressione immagini (toolchain assente nell'ambiente; le varianti WebP esistono già e sono servite via `<picture>`).
> - Cluster **Via Francigena** 3 spoke; **arrivi** completati; **Tuscia** rafforzata; **stagionali** da "solo estate" a 5 pagine; **esperienziali** nuovo blocco; **pagine concise** rinforzate; **hub** ripuliti dalle sezioni impilate. Totali aggiornati sotto. Prossimo sprint esecutivo proposto: **G** (audit finale + go-live).
...
| Internal linking | Buono | ~~hub con 3–4 sezioni "correlate" impilate~~ → **consolidate (Sprint F)** in blocco "Continua a esplorare" su 13 pagine | — | risolto |
...
| UX hub | Buona | ~~sezioni correlate ridondanti~~ → **risolto (Sprint F)** | — | griglia responsive unica |
| Schema GEO | Tutti validi | ~~`galleria.html` priva di JSON-LD~~ → **`ImageGallery` aggiunto (Sprint F)** | — | 25 ImageObject reali |
| Performance img | Buona | gallery JPG 250–450 KB; ricompressione non possibile (no toolchain) | P3 | WebP già servito via `<picture>`; ricomprimere in pipeline esterna |
